...
| Browser E2E runners | Playwright and Cypress | dependencies, owned `playwright.config.*` or `cypress.config.*`, static discovery fields, `*.spec.*` and `*.cy.*` files, plus bounded literal request-to-route evidence |
...
Browser route evidence is deliberately narrower than module evidence. An auditable `http-route` file can receive `browser-route-match` provenance with `indirect` strength only when it contains a literal `app`, `router`, `server`, `fastify`, `hono`, or `*Router` method registration and a recognized Playwright/Cypress test issues the same literal origin-relative path with the same HTTP method. Playwright `page.goto` and Cypress `cy.visit` count as `GET`; statically named Playwright `request` fixtures and Cypress `cy.request` calls preserve their explicit method. Query strings and fragments are removed before exact path comparison.
...
Large-suite analysis indexes module paths and caches relative resolution, resolved barrel exports, and declared export names for one audit. `npm run javascript:performance:check` locks a generated 400-source/200-test package to 200 covered targets, 200 untested targets, and 200 evidence relationships under a 5,000 ms audit budget. The deliberately broad budget is a regression ceiling across supported CI platforms, not a promise that arbitrary repositories finish within five seconds. Standardized three-run measurements now pass for all three pinned JavaScript/TypeScript corpus cases, recording their median durations, evidence counts, and stable normalized audit digests.
...
- Playwright and Cypress filenames alone do not count as module coverage. When either runner is present, filename-only evidence is suppressed across the repository because browser tests often use locally extended fixtures.
- An exact literal browser request and static HTTP route registration can provide weaker `browser-route-match` evidence. It credits only the route-registration file and does not claim that handlers, services, client modules, or other runtime dependencies were reached.
...
- runtime-only module loading, generated import maps, dynamic browser URLs, route parameters, route mounts/prefix composition, request object options, renamed/custom browser fixtures, React/router configuration, and browser reachability beyond an exact literal HTTP route registration
