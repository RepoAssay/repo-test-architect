...
The seventh-adapter spike is now underway with an experimental C#/.NET baseline for one static SDK-style test project or one unique literal production/test edge, including exact literal multi-target membership, finite project-local target-conditioned package predicates, and a selected pair amid unrelated projects, bounded nearest-file `Directory.Build.props` metadata and static `Directory.Packages.props` versions, xUnit/NUnit/MSTest discovery, exact project-file commands, project-qualified source ownership, runnable-body-owned unique-type calls with well-known `System` collision guards, stable concrete-local or exact immutable test-field receivers, bounded result and framework assertion usage including provenance-backed NUnit Legacy `ClassicAssert`, and one same-class private-static test-helper hop. The first five pinned probes validate receiver precision, unique-edge selection, inherited frameworks, central versions, and literal multi-target ownership. The sixth validates S7.Net's finite target-package predicate, direct-result assertions, runnable-body ownership, exception semantics, selected MSTest collection assertions, bounded helper reachability, and a false `System.DateTime` collision with a zero-blocker stable audit, and records the exact native command's x64/Mono/Snap7 portability findings on Apple Silicon. The seventh validates six-way repository routing, shared central packages, NUnit 5 discovery, conservative framework-callback evidence, exact NUnit Legacy assertion ownership, and 238 passing .NET 10 tests on the official NUnit samples. Solution ownership, broader evaluated MSBuild ownership, framework compatibility inference, transitive project edges, deeper helper/result flow, conditional/overridden central versions, and broader test-platform variants remain later slices; see [C# Experimental Support](csharp-alpha-support.md).
