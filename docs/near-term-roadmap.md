...
The seventh-adapter spike is now underway with an experimental C#/.NET baseline for one static SDK-style test project or one unique literal production/test edge, including exact literal multi-target membership, finite project-local target-conditioned package predicates, and a selected pair amid unrelated projects, bounded nearest-file `Directory.Build.props` metadata and static `Directory.Packages.props` versions, xUnit/NUnit/MSTest discovery, exact project-file commands, project-qualified source ownership, runnable-body-owned unique-type calls, stable concrete-local or exact immutable test-field receivers, one stable direct-call, receiver-call, or inline `out var` result assertion, sole-call MSTest expected exceptions, and exact framework exception assertion lambdas. The first five pinned probes validate receiver precision, unique-edge selection, inherited frameworks, central versions, and literal multi-target ownership. The sixth validates S7.Net's finite target-package predicate, direct-result assertions, runnable-body ownership, and exception assertion semantics with a zero-blocker stable audit, and records the exact native command's x64/Mono/Snap7 portability findings on Apple Silicon. Solution ownership, broader evaluated MSBuild ownership, framework compatibility inference, transitive project edges, helper reachability, deeper result flow, conditional/overridden central versions, and broader test-platform variants remain later slices; see [C# Experimental Support](csharp-alpha-support.md).
