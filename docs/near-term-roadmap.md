...
The seventh-adapter spike is now underway with an experimental C#/.NET baseline for one static SDK-style test project or one unique literal production/test edge, including a selected pair amid unrelated projects, bounded xUnit/NUnit/MSTest discovery, exact project-file commands, project-qualified source ownership, unique-type calls, stable concrete-local or exact immutable test-field receivers, and one assigned or inline `out var` result assertion. The first pinned probe validates local receiver precision; the second selects Sharp Cast's exact test/library edge from a four-project repository, recovers its two field-backed converters, upgrades three `out`-backed relationships to asserted, and passes all 165 native tests without absorbing the UI or benchmark projects. Solution ownership, evaluated MSBuild ownership, transitive project edges, deeper result flow, and broader test-platform variants remain later slices; see [C# Experimental Support](csharp-alpha-support.md).
