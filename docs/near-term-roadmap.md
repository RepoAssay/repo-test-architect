...
Rust has reached supported parity. Ruby is now the active experimental adapter: its conventional Bundler/Minitest/RSpec foundation is checked in with native-fixture, shared-conformance, implementation-coverage, golden, model, and generated performance gates. The pinned rubyzip audit passed 412 native tests, converted exact runner and root-gemspec ownership into regression-backed rules, locked three-edge require/unique-constant attribution, and now separates its unchanged 80-link graph into asserted, called, reference-only, and naming evidence through exact source-owned singleton methods. Broader RSpec pressure is the next live comparison before instance-receiver or Rails work. PHP then Elixir remain the provisional next-adapter sequence, while representative public repositories and concrete requests can still reorder demand.
