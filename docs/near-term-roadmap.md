...
The active adapter priority is bringing experimental Rust to supported parity. After that, the provisional new-adapter sequence is Ruby, PHP, then Elixir. The sequence is directional rather than a release promise: representative public repositories and concrete adapter requests can reorder those ecosystems when they provide stronger evidence of user demand.
