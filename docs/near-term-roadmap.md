...
Rust has reached supported parity. Ruby is now the active experimental adapter: its conventional Bundler/Minitest/RSpec foundation is checked in with native-fixture, shared-conformance, implementation-coverage, golden, model, and generated performance gates. The pinned rubyzip audit passed 412 native tests and locks exact runner, root-gemspec, require/constant, singleton-call, assertion, and direct immutable constructor-local receiver rules. The pinned Faraday audit passed 639 native RSpec examples and now locks one exact root `.rspec` helper plus exact one-line `let`/`subject` constructor receivers, improving three reviewed relationships while excluding generated service methods. The third Factory Bot pin passed 764 adapter-selected RSpec examples plus its broader upstream task and locks nearest exact constant-owned `described_class`; its unchanged memoized-receiver comparison also protects constructor-block, shared-example, implicit-matcher, generated, and inherited boundaries. Broader conventional ownership is the next evidence decision before helper/factory returns, shared-example binding, Rails, and the formal promotion corpus. PHP then Elixir remain the provisional next-adapter sequence, while representative public repositories and concrete requests can still reorder demand.
