...
The seventh-adapter spike is now underway with an experimental C#/.NET baseline for one static SDK-style test project or one unique literal production/test edge, including exact literal multi-target membership and a selected pair amid unrelated projects, bounded nearest-file `Directory.Build.props` metadata and static `Directory.Packages.props` versions, xUnit/NUnit/MSTest discovery, exact project-file commands, project-qualified source ownership, unique-type calls, stable concrete-local or exact immutable test-field receivers, and one assigned or inline `out var` result assertion. The first pinned probe validates local receiver precision; the second selects Sharp Cast's exact test/library edge from a four-project repository and passes all 165 native tests; the third selects Glob's inherited-framework edge from five projects and passes 179 tests under the documented local runtime roll-forward; the fourth validates central versions and passes all 118 EFCore.CheckConstraints tests; the fifth validates a `net8.0;net9.0;net10.0` production target superset against a `net10.0` test project and passes all 49 ErrorOrAspNetCoreExtensions tests. Solution ownership, broader evaluated MSBuild ownership, framework compatibility inference, transitive project edges, deeper result flow, conditional/overridden central versions, and broader test-platform variants remain later slices; see [C# Experimental Support](csharp-alpha-support.md).
