...
The first shared-kernel planning, latency-instrumentation, and migration slices are complete. The [Shared Audit Kernel Inventory](shared-audit-kernel-inventory.md) now backs a private portable-path and policy-driven deterministic text traversal primitive used only by PHP and Elixir. Each adapter still owns its ignored names, nested Composer/Mix boundary, file inclusion, lexical, build, framework, command, classification, and evidence semantics. Swift and Python expose callback-only timings for five ordered audit phases without changing public artifacts or diagnostics. The next measured slice is to profile the exact Swift Package Index Server and Django pins, then optimize only the adapter-local phase proven to dominate.
