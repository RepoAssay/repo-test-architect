...
The first shared-kernel planning, latency-instrumentation, migration, and exact-pin profiling slices are complete. The [Shared Audit Kernel Inventory](shared-audit-kernel-inventory.md) now backs a private portable-path and policy-driven deterministic text traversal primitive used only by PHP and Elixir. Each adapter still owns its ignored names, nested Composer/Mix boundary, file inclusion, lexical, build, framework, command, classification, and evidence semantics. The [Swift/Python phase report](swift-python-phase-profile-2026-08.md) shows that traversal is negligible at both large pins: Swift is dominated by repeated test import/masking/symbol work inside evidence joining, while Django is dominated by repeated test/support parsing across consumers. The next bounded slice is an immutable Swift test-fact index, followed by a separate Python parsed-file cache.
