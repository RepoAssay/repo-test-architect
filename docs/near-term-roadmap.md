...
The first shared-kernel planning, latency-instrumentation, migration, exact-pin profiling, and both bounded adapter optimization slices are complete. The [Shared Audit Kernel Inventory](shared-audit-kernel-inventory.md) now backs a private portable-path and policy-driven deterministic text traversal primitive used only by PHP and Elixir. Each adapter still owns its ignored names, nested Composer/Mix boundary, file inclusion, lexical, build, framework, command, classification, and evidence semantics. The [Swift/Python phase report](swift-python-phase-profile-2026-08.md) records how immutable Swift test facts reduced the exact Swift Package Index Server median from 14,443 ms to 725 ms and how an immutable Python parsed test/support-file index reduced Django from 3,823 ms to 2,129 ms. Both retain their exact canonical artifacts. The next ranked portfolio slice is the bounded non-shipping executor evaluation.
