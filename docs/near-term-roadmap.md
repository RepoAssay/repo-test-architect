...
The seventh-adapter spike is now underway with an experimental C#/.NET baseline for one static SDK-style test project or one literal production/test project pair, bounded xUnit/NUnit/MSTest discovery, exact project-file commands, project-qualified source ownership, and unique-type call evidence. The first pinned live probe validates the literal pair boundary and points to bounded instance-receiver and local-result flow as the next precision slice. Solution graphs, evaluated MSBuild ownership, transitive project edges, and broader test-platform variants remain later slices; see [C# Experimental Support](csharp-alpha-support.md).
