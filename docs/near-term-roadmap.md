...
Rust has reached supported parity. Ruby is now the active experimental adapter: its conventional Bundler/Minitest/RSpec foundation is checked in with native-fixture, shared-conformance, implementation-coverage, golden, model, and generated performance gates. The pinned rubyzip audit passed 412 native tests and locks exact runner, root-gemspec, require/constant, call, and assertion rules. The pinned Faraday audit passed 639 native RSpec examples and converted one exact root `.rspec` helper into 32 load-aware relationships without changing its 8/22 candidate split or crediting memoized service receivers. A direct immutable constructor-to-local receiver slice is the next bounded evidence candidate before RSpec memoization, helpers, shared examples, Rails, and the three promotion roles. PHP then Elixir remain the provisional next-adapter sequence, while representative public repositories and concrete requests can still reorder demand.
