...
The experimental Rust spike now covers conventional Cargo packages, literal repository-contained workspace members with exact package commands, the built-in `#[test]` harness, inline `#[cfg(test)]` modules, exact integration-test crate-module imports, native Cargo verification, golden artifacts, shared conformance, and a generated 400-source/200-test semantic/performance gate. Two additional representative live roles and broader syntax/evidence pressure remain before promotion; globbed or incomplete workspaces, custom harnesses, nonstandard async/property frameworks, and deeper Rust module/re-export ownership remain later slices. See [Rust Experimental Support](rust-alpha-support.md).
