...
Ruby and PHP have reached supported parity at their bounded matrices. Elixir now has an experimental foundation plus passing conventional-library and framework-heavy live roles against Jason and Plug: exact MixProject/Mixfile ownership, conventional or app-prefixed module/protocol ownership with bounded acronym and terminal-plural preservation, app-owned ExUnit discovery with static startup options, test-body-scoped direct evidence, safe `mix test` selection, a native fixture, shared conformance, implementation coverage, golden artifacts, packaging, and generated 400-source/200-test pressure. The next Elixir slice should select a difficult project- or command-ownership repository, with framework reachability remaining a separate explicit decision.
