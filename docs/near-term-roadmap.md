...
The seventh-adapter spike is now underway with an experimental C#/.NET baseline for one static SDK-style test project or one literal production/test project pair, bounded xUnit/NUnit/MSTest discovery, exact project-file commands, project-qualified source ownership, unique-type calls, and stable concrete-local receiver/one-result assertion flow. The first pinned live probe validates both the pair boundary and the receiver precision gain without changing candidate or relationship counts. Solution graphs, evaluated MSBuild ownership, transitive project edges, and broader test-platform variants remain later slices; see [C# Experimental Support](csharp-alpha-support.md).
