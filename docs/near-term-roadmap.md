...
Rust has reached supported parity. Ruby is now the active experimental adapter: its first conventional Bundler/Minitest/RSpec foundation slice is checked in with native-fixture, shared-conformance, implementation-coverage, golden, model, and generated performance gates. The first pinned rubyzip audit passed 412 native tests and converted exact `Minitest::TestTask` plus root-gemspec runner ownership into regression-backed rules. PHP then Elixir remain the provisional next-adapter sequence, while representative public repositories and concrete requests can still reorder demand.
