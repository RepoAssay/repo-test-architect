...
Rust has reached supported parity. Ruby is now the active experimental adapter: its conventional Bundler/Minitest/RSpec foundation is checked in with native-fixture, shared-conformance, implementation-coverage, golden, model, and generated performance gates. The pinned rubyzip audit passed 412 native tests, first converted exact `Minitest::TestTask` plus root-gemspec runner ownership into regression-backed rules, and now locks three-edge literal require/unique-constant evidence that moves four source files into covered evidence without crediting the internal filesystem file. PHP then Elixir remain the provisional next-adapter sequence, while representative public repositories and concrete requests can still reorder demand.
