...
| [Ruby rubyzip live validation](ruby-rubyzip-validation-report.md) | Ruby, conventional Bundler gem, Minitest | `rubyzip/rubyzip` at `4209b02` | root gemspec runner ownership, exact Minitest Rake command, native tests, bounded require/constant/call/assertion and constructor-local receiver evidence | current; first Ruby live probe and evidence pressure pin |
| [Ruby Faraday live validation](ruby-faraday-validation-report.md) | Ruby, conventional Bundler HTTP gem, RSpec | `lostisland/faraday` at `3725183` | root `.rspec` helper ownership, exact three-edge load evidence, native RSpec, constructor-local and exact memoized receiver boundaries | current; second Ruby live probe and RSpec pressure pin |
...
| [Ruby Diplomat live validation](ruby-diplomat-validation-report.md) | Ruby, complete named two-gemspec Bundler project, RSpec | `WeAreFarmGeek/diplomat` at `313f94f` | exact aggregate package ownership, command recovery, per-file `spec_helper` reachability, bounded dynamic-fan-out exclusion, native RSpec | current; fourth Ruby live probe and multi-gemspec/helper ownership pin |
