...
| [C# multi-target live validation](csharp-multi-target-validation-report.md) | C#, literal multi-target SDK production project, single-target xUnit project | `matthewrosse/ErrorOrAspNetCoreExtensions` at `b4244dd` | exact target membership, central-package composition, exact command, native test comparison | current; fifth C# live probe |
| [C# target-conditioned packages live validation](csharp-package-conditions-validation-report.md) | C#, finite target package predicate, literal multi-target MSTest pair | `S7NetPlus/s7netplus` at `534d9fd` | target-condition truth table, mixed package XML forms, exact command, native host portability findings | current; sixth C# live probe |
| [C# NUnit 5 samples live validation](csharp-nunit-samples-validation-report.md) | C#, six independent SDK test projects, NUnit 5, central packages | `nunit/nunit-csharp-samples` at `2058fb6` | repository-wide project routing, shared central metadata, exact per-project commands, native multi-target portability finding | current; seventh C# live probe |
| [C# Microsoft.Testing.Platform v2 live validation](csharp-mtp-v2-validation-report.md) | C#, unique literal xUnit pair, native MTP v2 | `Lukdrasil/StepUpLogging` at `958a048` | root runner ownership, .NET 10 command context, native test comparison | current; eighth C# live probe |
| [C# MSTest.Sdk v4 live validation](csharp-mstest-sdk-validation-report.md) | C#, versioned MSTest SDK, native MTP, literal project edge and linked sources | `dorssel/usbipd-win` at `15e5f85` plus two complementary probes | zero-blocker SDK, runner, target-alias, and compile-glob ownership; exact command; local/upstream native comparison | current; ninth C# live probe |
...
| [Rust Starship live validation](rust-starship-validation-report.md) | Rust, framework-heavy CLI application, built-in inline tests | `starship/starship` at `7946f2d` | production versus exact `cfg(test)` module ownership, native application tests, stable evidence correction | current; second Rust live probe and framework-heavy promotion role |
| [Rust serde_json live validation](rust-serde-json-validation-report.md) | Rust, standalone parser library, built-in integration tests and doctests | `serde-rs/json` at `a3e9758` | exact crate-root re-export evidence, conservative doctest/macro/receiver boundaries, native parser tests | current; third Rust live probe and conventional-library promotion role |
| [Ruby rubyzip live validation](ruby-rubyzip-validation-report.md) | Ruby, conventional Bundler gem, Minitest | `rubyzip/rubyzip` at `4209b02` | root gemspec runner ownership, exact Minitest Rake command, native tests, bounded require/constant/call/assertion and constructor-local receiver evidence | current; conventional-library promotion role |
| [Ruby Faraday live validation](ruby-faraday-validation-report.md) | Ruby, conventional Bundler HTTP gem, RSpec | `lostisland/faraday` at `3725183` | root `.rspec` helper ownership, exact three-edge load evidence, native RSpec, constructor-local and exact memoized receiver boundaries | current; framework-heavy promotion role |
| [Ruby Factory Bot live validation](ruby-factory-bot-validation-report.md) | Ruby, conventional Bundler gem, RSpec and Cucumber upstream | `thoughtbot/factory_bot` at `967d128` | exact constant-owned `described_class`, nearest nested-group ownership, memoized-receiver negative control, native upstream comparison | current; third Ruby live probe and class-oriented RSpec pressure pin |
| [Ruby Diplomat live validation](ruby-diplomat-validation-report.md) | Ruby, complete named two-gemspec Bundler project, RSpec | `WeAreFarmGeek/diplomat` at `313f94f` | exact aggregate package ownership, command recovery, per-file `spec_helper` reachability, bounded dynamic-fan-out exclusion, native RSpec | current; difficult-ownership promotion role |
| [Ruby helper/factory return live validation](ruby-helper-return-validation-report.md) | Ruby, conventional Bundler gems, RSpec | `dinie-tech/sdk-ruby` at `1104830` and `somleng/cgrates-ruby` at `a9aacc4` | exact same-group helper returns, source-factory regressions, lexical-constant negative control, native RSpec | current; fifth Ruby live probe and receiver-return pressure pin |
| [Ruby Licensed post-promotion validation](ruby-licensed-post-promotion-validation-report.md) | Ruby, Bundler gem, Minitest spec DSL, setup-partitioned suites | `licensee/licensed` at `2db4c2a` | blind supported-boundary audit, exact blocker behavior, default-command prerequisite pressure, passing upstream core comparison | current; first blind Ruby post-promotion probe |
| [PHP brick/math live validation](php-brick-math-validation-report.md) | PHP, Composer, PSR-4, PHPUnit | `brick/math` at `b61d8e6` | required bootstrap environment withholding, one-hop test-base ownership, exact exception/result assertions, native matrix comparison | current; conventional-library promotion role |
| [PHP Guzzle live validation](php-guzzle-validation-report.md) | PHP, Composer, PSR-4, PHPUnit, HTTP services | `guzzle/guzzle` at `9b200fc` | Make prerequisite orchestration, bare-command withholding, native service workflow comparison, bounded direct evidence | current; framework-heavy promotion role |
| [PHP Ramsey UUID live validation](php-ramsey-uuid-validation-report.md) | PHP, Composer script graph, PSR-4, PHPUnit | `ramsey/uuid` at `da5b521` | bounded quality aliases, exact autoloaded function-file ownership, native lint/benchmark/static-analysis/test workflow | current; difficult-ownership promotion role |
| [PHP Monolog post-promotion validation](php-monolog-post-promotion-validation-report.md) | PHP, Composer, mirrored PSR-4 namespaces, source-owned PHPUnit base | `Seldaek/monolog` at `57eb102` | installed-package blind audit, exact one-hop source-base ownership, same-namespace evidence, transparent failing upstream/native comparison | current; first blind PHP post-promotion probe |
| [Elixir Jason live validation](elixir-jason-validation-report.md) | Elixir, conventional Mix library, ExUnit/doctests/properties | `michalmuskala/jason` at `4ede428` | Mixfile and flat app ownership, protocol and grouped-alias evidence, exact command, native suite, dependency-tree exclusion | current; first Elixir live probe and conventional-library role |
| [Elixir Plug live validation](elixir-plug-validation-report.md) | Elixir, framework-heavy HTTP library, ExUnit/doctests | `elixir-plug/plug` at `2463704` | acronym and terminal-plural ownership, static ExUnit startup options, exact command, native suite, conservative framework boundary | current; second Elixir live probe and framework-heavy role |
| [Elixir Absinthe live validation](elixir-absinthe-validation-report.md) | Elixir, macro-heavy GraphQL toolkit, local ExUnit wrappers, Mix tasks | `absinthe-graphql/absinthe` at `de0e411` | literal compiled-support wrapper chains, Mix task ownership, repeated exact modules, acronym namespaces, exact command, native suite | current; third Elixir live probe and difficult-ownership role |
