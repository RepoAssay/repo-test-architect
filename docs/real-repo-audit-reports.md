...
| [Ruby Faraday live validation](ruby-faraday-validation-report.md) | Ruby, conventional Bundler HTTP gem, RSpec | `lostisland/faraday` at `3725183` | root `.rspec` helper ownership, exact three-edge load evidence, native RSpec, constructor-local receiver and generated-method boundary comparison | current; second Ruby live probe and RSpec pressure pin |
| [Ruby Factory Bot live validation](ruby-factory-bot-validation-report.md) | Ruby, conventional Bundler gem, RSpec and Cucumber upstream | `thoughtbot/factory_bot` at `967d128` | exact constant-owned `described_class`, nearest nested-group ownership, native RSpec and broader upstream suite comparison | current; third Ruby live probe and class-oriented RSpec pressure pin |
