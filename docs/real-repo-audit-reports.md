...
| [Ruby Diplomat live validation](ruby-diplomat-validation-report.md) | Ruby, complete named two-gemspec Bundler project, RSpec | `WeAreFarmGeek/diplomat` at `313f94f` | exact aggregate package ownership, command recovery, native RSpec, unchanged naming-only evidence boundary | current; fourth Ruby live probe and multi-gemspec ownership pin |
