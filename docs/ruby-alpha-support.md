...
- receivers from RSpec `let`/`subject`, `described_class`, helpers, setup, factories, parameters, fields, wrapping/chaining, or constructor overrides; attributes/delegators, `extend self`, inherited/mixed-in/generated methods, shared examples, custom assertion methods, multiline assertion flow, or deeper result flow
...
The next evidence decision should use a third pinned live role to choose between exact RSpec memoized receiver identity and broader conventional project ownership before attempting helpers, shared examples, deeper result flow, or Rails ownership.
