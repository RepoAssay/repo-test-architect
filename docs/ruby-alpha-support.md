...
Exact group-owned `described_class`, direct one-line constructor memos, and complete named root-gemspec ownership are now bounded. Exact conventional per-file test-helper loading is the next comparison point before attempting multiline/eager memos, helper/factory returns, shared examples, implicit matcher semantics, deeper result flow, or Rails ownership.
...
The fourth live probe, [`WeAreFarmGeek/diplomat`](https://github.com/WeAreFarmGeek/diplomat) at `313f94ff2582c7eb161e2aa775182db10ad14285`, passed 281 native RSpec examples. Its two exact named root gemspecs now form one high-confidence Ruby gem with `bundle exec rspec`, while its unchanged 9/20/3 candidate split and 20 naming links preserve the unresolved explicit `require "spec_helper"` boundary. See the [Diplomat Live Validation Report](ruby-diplomat-validation-report.md).
