...
Exact group-owned `described_class`, direct one-line constructor memos, complete named root-gemspec ownership, and conventional per-file RSpec `spec_helper` loading are now bounded. Exact helper/factory receiver returns are the next comparison point before attempting multiline/eager memos, shared examples, implicit matcher semantics, deeper result flow, or Rails ownership.
