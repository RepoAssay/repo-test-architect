...
### 2026-07-31: promote the bounded Rust adapter to supported alpha

Decision: promote Rust from `experimental` to `supported` for the documented Cargo package/workspace-member boundary. As the final evidence slice, follow one exact unconditional library-root `pub use crate::<literal module>::<symbol>` re-export into an integration test's direct imported call. Preserve aliases, require unique module and callable ownership, and reject wildcard, conditional, relative, chained, trait, receiver, macro, helper, and doctest inference.
