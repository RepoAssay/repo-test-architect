...
The later assertion-usage and parser-scope slices retain those 287 relationships and all candidate classifications while upgrading 56 exact Testify relationships. Its then-current three-run median was 662 ms and normalized digest was `ef7f291da706cf6f59f0f73c9ada5c455e9914c81edac203491b9387874eba61`; details are recorded in the [Go Assertion-Usage Validation Report](go-assertion-validation-report.md) and [Go Parser-Scoped Binding Validation Report](go-parser-scope-validation-report.md).

The later receiver/callable-ownership slice removes file-wide indirect leakage and enables exact multi-callable bodies. Zap now reports `1 / 49 / 9` candidates and 259 relationships, including exact receiver methods and the body-owned `zaptest.Timeout` to `ztest.Timeout` edge. Current measurements are recorded in the [Go Receiver And Callable Ownership Validation Report](go-callable-ownership-validation-report.md).
