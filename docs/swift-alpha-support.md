...
| Commands | `swift test`, project/workspace/scheme/test-plan-aware `xcodebuild test`, and `bazel test //...`; ambiguous shared schemes or test plans block command selection | package, sole or scheme-matching project/workspace, container-matching shared scheme, scheme-default or otherwise unambiguous test plan, and `swift_test` markers |
...
- Xcode scheme and test-plan content is not fully interpreted; ambiguous selections now block commands, while custom build configurations, destinations, test target filters, and dynamically generated schemes remain unresolved
