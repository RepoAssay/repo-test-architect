...
- Kotlin/JVM support is bounded to conventional Gradle/Maven module roots, settings-owned Gradle aggregates, and directly declared Maven reactors with standard JVM source sets, explicitly exported module-dependency chains, JUnit/`kotlin.test`, Gradle/JUnit Platform Kotest common specs, and method-level TestNG with direct conventional execution; computed/nested Maven reactors, inherited/dynamic or non-exported dependencies, custom/composite Gradle graphs, Android, multiplatform, and advanced Kotest/TestNG or other framework semantics remain excluded or blocked
