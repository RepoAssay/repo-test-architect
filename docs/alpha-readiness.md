...
- Kotlin/JVM support is bounded to conventional Gradle/Maven module roots, settings-owned Gradle aggregates, and directly declared Maven reactors with standard JVM source sets, explicitly exported module-dependency chains, JUnit/`kotlin.test`, and Gradle/JUnit Platform Kotest `FunSpec`, `StringSpec`, or `ShouldSpec`; computed/nested Maven reactors, inherited/dynamic or non-exported dependencies, custom/composite Gradle graphs, Android, multiplatform, and other Kotest/framework semantics remain excluded or blocked
