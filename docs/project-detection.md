...
One C# production/test pair collapses to its common audit root only when the repository contains exactly one statically identifiable test-to-production edge through one unconditional literal relative `ProjectReference`. Unrelated SDK projects remain separate detected roots. Dynamic, conditional, absolute, escaping, missing, additional-reference, overlapping, and multiple-valid-edge graphs remain uncollapsed so repository-wide analysis cannot invent aggregate ownership.
