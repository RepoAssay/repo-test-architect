...
- experimental C# SDK project audits with one static root test `.csproj` or one literal production/test project pair, xUnit/NUnit/MSTest attributed-test discovery, exact test-project `dotnet test` commands, project-qualified source ownership, direct unique-type call evidence across the verified edge, naming fallback evidence, generated/wiring/model/contract deferral, shared conformance, golden artifacts, model locking, and native .NET 10 fixture validation
