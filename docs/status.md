...
- experimental C# SDK project audits with one static root `.csproj`, xUnit/NUnit/MSTest attributed-test discovery, exact project-file `dotnet test` commands, collocated source ownership, direct unique-type call evidence, naming fallback evidence, generated/wiring/model/contract deferral, shared conformance, golden artifacts, model locking, and native .NET 10 fixture validation
