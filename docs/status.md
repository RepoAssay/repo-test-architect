...
Unreleased preparation on 2026-09-06 adds experimental [Dart and Flutter support](dart-support.md), bringing the registry to eleven adapters: ten supported and one experimental. Dart is the final planned language addition before stable release. The [distribution ledger](distribution-metrics.md#pre-release-snapshot--2026-09-06) has current npm and GitHub evidence; public tags remain `latest: 0.3.0` and `beta: 1.0.0-beta.1`. Stable publication has not occurred.

**Release review hold:** the [September adapter re-evaluation](adapter-re-evaluation-2026-09.md) found defects across seven previous adapters. The [first repair phase](lexical-evidence-hardening-2026-09.md) fixes the reproduced lexical evidence cases. The [second repair phase](ownership-hardening-2026-09.md) fixes external symlink reads, duplicate nested ownership, PHP test-base identity and Kotlin/Swift inactive command metadata, with regressions and unchanged five-run audits of all eighteen affected corpus pins. The reproduced implementation findings are repaired; exact-commit Linux/Windows/macOS validation and the existing release-policy review remain outstanding.
