...
- experimental Rust Cargo package audits with literal workspace-member ownership, exact `cargo test -p <package>` commands, aggregate-only virtual roots, built-in `#[test]`, inline `#[cfg(test)]`, exact integration-test crate-module imports, golden artifacts, model-consistency locking, and shared downstream conformance
