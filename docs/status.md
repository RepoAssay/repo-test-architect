...
Repo Test Architect currently has supported deterministic JavaScript/TypeScript, Python, Swift, bounded Kotlin/JVM, bounded Go, and bounded C# audit pipelines. Go support includes conventional modules, literal repository-contained `go.work` ownership, and explicit static build-target selection. C# support covers one conventional SDK-style test project or one unique literal production/test edge, including exact literal multi-target membership, finite project-local target-conditioned package predicates, and a selected pair amid unrelated projects. The bounded Rust Cargo package/workspace-member pipeline remains experimental.
...
- runtime adapter registry with supported `csharp`, `javascript`, `go`, `kotlin`, `python`, and `swift` adapters plus the experimental bounded `rust` adapter
...
- deterministic generated 400-source/200-test performance and evidence-count gates for all six supported adapters plus experimental Rust
...
1. Shared validation-corpus manifest, scorecard schema, semantic checker, adapter-conformance helper, and per-adapter implementation coverage floors are complete. All 18 pinned cases across six supported adapters pass standardized stability and performance measurement; corpus cases can preserve bounded adapter options such as an explicit Go target.
