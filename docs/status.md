...
- runnable-body-owned Ruby singleton/constructor calls, nearest exact constant-owned RSpec `described_class`, and direct immutable constructor-local, exact one-line RSpec `let`/`subject`, exact source-factory, or exact same-group RSpec helper-return instance calls with selected Minitest/RSpec inline and one-stable-result assertion usage
