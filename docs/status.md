...
- experimental C# SDK project audits with one static root test `.csproj` or one unique literal production/test edge selected without absorbing unrelated projects, xUnit/NUnit/MSTest attributed-test discovery, exact test-project `dotnet test` commands, project-qualified source ownership, direct unique-type calls plus stable concrete-local receiver and one-result assertion evidence across the verified edge, naming fallback evidence, generated/wiring/model/contract deferral, shared conformance, golden artifacts, model locking, native .NET 10 fixture validation, and pinned `aelassas/tdd` and `jjosh102/sharp-cast` live probes
