...
- experimental Rust Cargo package audits with literal workspace-member ownership, static package-local `[lib].path` and `[[bin]].path` source ownership, recursive literal `mod` and static `#[path]` module graphs, exact `cargo test -p <package>` commands, aggregate-only virtual roots, built-in `#[test]`, static explicit built-in test targets, inline `#[cfg(test)]`, exact integration-test crate-module imports, golden artifacts, model-consistency locking, shared downstream conformance, and the pinned ripgrep live probe
