...
- bounded Ruby `ruby-constant-reference` evidence through exact literal repository-owned load graphs, including one uniquely owned root `.rspec` helper edge, direct/referenced strength, reachable-owner ambiguity rejection, and pinned rubyzip/Faraday pressure
- runnable-body-owned Ruby singleton/constructor calls and direct immutable constructor-local instance calls with selected Minitest/RSpec inline and one-stable-result assertion usage
