...
`1.0.0-beta.2` is prepared for review on 2026-09-06, not published. Its [release notes](release-notes-v1.0.0-beta.2.md) cover experimental [Dart and Flutter support](dart-support.md), the final planned language addition before stable release, plus adapter trust and security fixes. The registry has eleven adapters: ten supported and one experimental. The [distribution ledger](distribution-metrics.md#pre-release-snapshot--2026-09-06) records public tags `latest: 0.3.0` and `beta: 1.0.0-beta.1`; preparing this candidate changes neither tag.

**Release review hold:** the [September adapter re-evaluation](adapter-re-evaluation-2026-09.md) findings are repaired by the [lexical](lexical-evidence-hardening-2026-09.md) and [ownership/command](ownership-hardening-2026-09.md) phases, including unchanged five-run audits of all eighteen ownership-repair corpus pins. PRs #274/#275 passed their updated Linux/Windows/macOS checks, and the merged implementation passed its Linux release gate. This newly versioned publication commit still requires the manually dispatched three-OS release matrix and the manual release checklist. No publication, Dart promotion, external-feedback waiver or stable approval is implied. Start the planned fourteen-day beta.2 clean window only when it is published.

Repo Test Architect currently has supported deterministic JavaScript/TypeScript, PHP, Python, Swift, bounded Kotlin/JVM, bounded Go, bounded C#, bounded Rust, bounded Ruby, and bounded Elixir audit pipelines. PHP support covers one root Composer/PSR-4 project with PHPUnit, exact bounded commands or safe command withholding, one-hop test bases below exact source or test mappings, literal namespaced function-file ownership, and conservative imported or same-namespace direct evidence. Go support includes conventional modules, literal repository-contained `go.work` ownership, and explicit static build-target selection. C# support covers one conventional SDK-style test project or one unique literal production/test edge, including exact literal multi-target membership, finite project-local target-conditioned package predicates, and a selected pair amid unrelated projects. Rust support covers conventional Cargo packages and literal workspace members with built-in tests, exact commands, literal module ownership, and bounded direct evidence. Ruby support covers one conventional Bundler project with `lib/`, one root gemspec or a complete exact named root-gemspec set, `Minitest::Test` `test_*` or RSpec discovery, bounded commands, root `.rspec` and exact per-file `spec_helper` edges within three-edge require/unique-constant evidence, exact singleton calls, exact constant-owned RSpec `described_class`, direct immutable constructor-local, exact one-line RSpec memoized, exact source-factory, exact same-group RSpec helper-return instance receivers, and exact same-file literal shared-example inclusion, bounded assertion usage, and naming fallback. Minitest spec DSL and setup-partitioned custom commands remain outside that boundary after the blind Licensed pressure pass. Elixir support covers one conventional Mix application with exact project, source, task, local ExUnit wrapper, test-body evidence, and safe `mix test` ownership while arbitrary Mix evaluation and framework reachability remain excluded.

Implemented:

- experimental Dart live validation across dart-lang/core, flutter/samples, and felangel/bloc: four native suites passing 2,064 tests, two native fixtures, a pinned three-role cohort, bounded indirect export reachability, and abstract-declaration deferral; see the [live validation report](dart-live-validation-report.md)
...
- runtime adapter registry with supported bounded `csharp`, `javascript`, `go`, `kotlin`, `php`, `python`, `ruby`, `rust`, and `swift` adapters
...
- deterministic generated 400-source/200-test performance and evidence-count gates for all ten supported adapters
...
- exact-pin Swift Package Index Server and Django phase/CPU profiling that rejects traversal as their latency target and selects separate adapter-local immutable test-fact slices
- immutable adapter-local Swift test facts for normalized imports, owners, masked code, local declarations, assertion bodies, and identifier positions, reducing the exact Swift Package Index Server median from 14,443 ms to 725 ms with the same canonical artifact and 265 evidence relationships
- immutable adapter-local Python test/support-file facts for masked content, function blocks, parsed functions, and resolved imports, reducing the exact Django median from 3,823 ms to 2,129 ms with the same canonical artifact and 4,935 evidence relationships
...
1. Shared validation-corpus manifest, scorecard schema, semantic checker, adapter-conformance helper, and per-adapter implementation coverage floors are complete. All 30 current supported-adapter pins pass standardized stability and performance measurement, including the three Elixir roles. Corpus cases can preserve bounded adapter options such as an explicit Go target and represent reviewed safe command withholding with `testCommand: null`.
