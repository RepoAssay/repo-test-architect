...
- bounded Go cross-package source pressure against `riverqueue/river` and `uber-go/zap`, including exact module-local imports, single-callable caller files, exported unique dependency functions, stable candidate classifications, and rejected deeper or ambiguous propagation
- bounded Go assertion-usage pressure across TOML, Chi, River, Zap, and Resty, including exact standard-library failure conditions, exact Testify imports, one-hop result bindings, stable candidate/relationship counts, and indexed performance at the prior River baseline
