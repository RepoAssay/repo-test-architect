...
| Commands | `pytest`, `python -m unittest`, Django `manage.py test`/`tests/runtests.py`, `uv run`, `poetry run`, explicitly configured `hatch test`, proven tox environments, and literal proven nox test sessions | detected framework and project-tool markers; Hatchling alone is treated only as a build backend, while Django/tox/nox commands require explicit runner evidence and competing proven runners produce a blocker |
| Test locations | root `test/`, `testing/`, and `tests/`, package-local variants, literal pytest `testpaths`, and repository-owned ancestor pytest configuration | conventional directories plus bounded `test_*.py`, `*_test.py`, Django-style `tests.py`, or configured `python_files` matches; inherited paths must stay inside the audited project |
...
Pytest discovery reads one configuration using pytest's precedence: `pytest.toml`/`.pytest.toml`, `pytest.ini`/`.pytest.ini`, `pyproject.toml`, `tox.ini`, then `setup.cfg`. A project-local configuration wins. For a nested project audit, the nearest matching ancestor configuration may be inherited only up to the detected repository root. Literal `testpaths` are resolved from that configuration's directory and must all remain inside the audited project; an escaping or unsupported path also makes the inherited selection unbounded. In either ambiguous case, the adapter keeps any locally provable evidence but withholds the test command, lowers confidence, and emits an ownership blocker. Simple basename `python_files` globs become authoritative test candidates, while helpers and `conftest.py` inside configured roots remain test support rather than production candidates. Absolute, parent-escaping, path-glob, bracket, and brace values are rejected instead of broadening the audit. See pytest's [configuration formats](https://docs.pytest.org/en/stable/reference/customize.html) and [Python collection settings](https://docs.pytest.org/en/stable/example/pythoncollection.html), plus setuptools [package discovery](https://setuptools.pypa.io/en/stable/userguide/package_discovery.html).

Runner selection associates a nox command only with the body of its literal `@nox.session` function, including an optional literal `name`. Unrelated build sessions and helper functions do not become test commands. One proven tox environment or nox test session remains selectable; multiple nox test sessions, or tox and nox together, withhold the command and emit a deterministic ambiguity blocker while preserving both setup signals.
...
- pytest configuration above the detected repository root, plugin-mutated discovery, `--ignore`/`norecursedirs`, and advanced glob semantics are not resolved; inherited `testpaths` spanning several project owners are blocked rather than split or guessed
- aliased/computed nox decorators, dynamic session names, parametrized tox environment selection, and repository-specific orchestration wrappers are not resolved
...
- alternate test roots require literal project-owned `testpaths`; custom filenames require simple basename `python_files` globs without bracket, brace, or path syntax
