...
- `csharp` as an experimental adapter for one static SDK-style .NET test project or one unique literal production/test project edge, including a selected pair amid unrelated projects and bounded nearest-file `Directory.Build.props` metadata, using xUnit, NUnit, or MSTest and bounded by the [C# experimental support matrix](csharp-alpha-support.md)
