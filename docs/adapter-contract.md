...
- `csharp` as an experimental adapter for one static SDK-style .NET test project or one unique literal production/test project edge, including a selected pair amid unrelated projects, bounded nearest-file `Directory.Build.props` metadata, and static `Directory.Packages.props` versions, using xUnit, NUnit, or MSTest and bounded by the [C# experimental support matrix](csharp-alpha-support.md)
...
- `rust` as an experimental adapter for conventional Cargo packages and literal repository-contained workspace members using the built-in test harness, bounded by the [Rust experimental support matrix](rust-alpha-support.md)
