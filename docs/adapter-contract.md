...
- `go` for conventional Go modules and literal repository-contained `go.work` members using the standard `testing` package, with optional explicit static build-target context, registered as experimental within the [Go support matrix](go-experimental-support.md)
