...
- `elixir` as an experimental bounded adapter for one conventional Mix application with literal app/path ownership and conventional ExUnit tests, bounded by [Elixir experimental support](elixir-alpha-support.md)
...
- `rust` for conventional Cargo packages and literal repository-contained workspace members using the built-in test harness, bounded by the [Rust alpha support matrix](rust-alpha-support.md)
