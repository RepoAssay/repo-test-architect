...
- `go` for conventional Go modules and literal repository-contained `go.work` members using the standard `testing` package, with exact top-level and explicitly typed receiver-method evidence plus optional explicit static build-target context, supported within the [Go support matrix](go-alpha-support.md)
