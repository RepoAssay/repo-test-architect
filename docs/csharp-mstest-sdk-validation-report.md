...
This slice admits exact MSTest.Sdk v4 test applications when the SDK version, Microsoft.Testing.Platform runner, and .NET 10+ context are all static and repository-owned. It rejects missing or pre-v4 versions, `UseVSTest`, helper libraries, inferred runner ownership, and dynamic MSBuild metadata. usbipd-win is now correctly started as an auditable pair; its root target alias and custom compile ownership remain separate follow-up slices.
