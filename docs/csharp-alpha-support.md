...
Literal pair selection now covers a unique test edge amid unrelated projects without claiming solution ownership, Sharp Cast proves exact test-class field receivers initialized through target-typed `new()` plus one stable inline `out var` result, Glob proves bounded nearest-file inherited metadata, EFCore.CheckConstraints proves bounded central versions, and ErrorOrAspNetCoreExtensions proves literal multi-target ownership. Broader MSBuild evaluation, framework compatibility inference, conditional or overridden central versions, and explicitly typed or multi-output flow remain pressure only when another pinned repository justifies them.
