...
| Receiver/result usage | A stable concrete local or exact private readonly concrete test field can prove `value.Method(...)`; fields use exact inline `new()`/`new Type(...)` or one assignment in the class's sole parameterless constructor. The direct call or one stable local result consumed by `Assert.*` or `.Should(...)` is `asserted` | Receiver/result reassignment, `ref`/`out`, mutable/static/interface/property/inherited/helper/partial field identity, multiple constructors, local shadows, nested local functions, deferred lambdas, and deeper result flow are rejected |
...
`examples/csharp-sdk-unique-pair` adds unrelated Worker and benchmark projects around one Pricing/Pricing.Tests edge. Detection collapses only the exact pair, direct audit excludes unrelated source, and the fixture locks the command plus asserted `private readonly` target-typed field evidence through golden and model-consistency artifacts. It passes:
...
- namespace, alias, partial-type joining, generic-type, overload, target-typed construction outside the exact supported field shape, receiver/result reassignment, mutable/static/interface/property/inherited/helper field identity, helper-return, nested local-function, deferred-lambda, `out`-result assertion flow, mock, reflection, source-generator, or dependency-graph resolution
- cross-project evidence outside the one verified production/test edge, or any transitive evidence

Literal pair selection now covers a unique test edge amid unrelated projects without claiming solution ownership, and Sharp Cast proves exact test-class field receivers initialized through target-typed `new()`. Its remaining JSON-converter usage makes bounded `out`-result assertion flow the clearest evidence pressure; inherited build/package metadata remains the next ownership pressure.
