...
- namespace, alias, partial-type joining, generic-type, overload, target-typed construction outside the exact supported field shape, receiver/result reassignment, mutable/static/interface/property/inherited/helper field identity, helper-return or helper-mediated source/assertion flow, predeclared/explicitly typed/multiple/nested `out` declarations, `ref`/`out` forwarding, nested local-function, deferred-lambda, mock, reflection, source-generator, or dependency-graph resolution
- cross-project evidence outside the one verified production/test edge, or any transitive evidence

Literal pair selection now covers a unique test edge amid unrelated projects without claiming solution ownership, Sharp Cast proves exact test-class field receivers initialized through target-typed `new()` plus one stable inline `out var` result, Glob proves bounded nearest-file inherited metadata, EFCore.CheckConstraints proves bounded central versions, ErrorOrAspNetCoreExtensions proves literal multi-target ownership, and S7.Net proves finite project-local target package predicates plus stable direct type-call results. Broader MSBuild evaluation, framework compatibility inference, conditional or overridden central versions, and explicitly typed or multi-output `out` flow remain pressure only when another pinned repository justifies them.
