...
11. Start C#/.NET after the bounded Rust spike reaches its intended comparison point. Planned; begin with conventional SDK-style `.csproj` ownership, standard test-project discovery, and exact `dotnet test` command selection. The provisional sequence after C#/.NET is Ruby, PHP, then Elixir, subject to representative repositories and concrete adapter-request demand.
