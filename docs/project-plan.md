...
12. Start Ruby as the next bounded adapter. In progress: the experimental conventional Bundler foundation owns `lib/`, runnable Minitest/RSpec conventions, exact bounded commands, one literal root `.rspec` helper edge within three-edge require/unique-constant evidence, unique-basename fallback, exact source-owned singleton calls with bounded assertion usage, one native fixture, and a generated 400-source/200-test gate. Pinned rubyzip command/constant/usage pressure and Faraday configured-helper/RSpec pressure are regression-backed; direct immutable instance receivers, broader RSpec helpers, Rails ownership, and the three promotion roles remain later slices in the [Ruby alpha support matrix](ruby-alpha-support.md). PHP then Elixir remain the provisional next-adapter order.
