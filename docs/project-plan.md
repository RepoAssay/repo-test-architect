...
5. Harden SwiftPM, Xcode, and Bazel ownership plus conservative Swift symbol evidence. Xcode workspace/project, shared-scheme, and test-plan ambiguity now blocks command selection, test-local shadows and same-named member calls no longer inflate source evidence, and source-file-unique static extension calls have a bounded explicit-receiver path. SwiftPM graph ownership and instance/inferred-member evidence remain active pressure.
