...
9. Start the fifth adapter after the four supported adapters meet the hardening definition of done. The bounded experimental Go spike now covers conventional modules plus literal repository-contained `go.work` members with module-local commands, standard-library tests, shared conformance, golden artifacts, model-consistency locking, and implementation coverage. Promotion remains gated by the checked-in [Go support matrix](go-experimental-support.md).
