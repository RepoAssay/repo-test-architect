...
11. Start C#/.NET after the bounded Rust spike reaches its intended comparison point. In progress; the first experimental slice audits one static SDK-style test `.csproj`, detects bounded xUnit/NUnit/MSTest attributed tests, selects an exact project-file `dotnet test` command, classifies collocated source, and emits direct unique-type call evidence. Separate production/test projects, solution graphs, and evaluated MSBuild ownership remain later slices in the [C# experimental support matrix](csharp-alpha-support.md). The provisional sequence after C#/.NET is Ruby, PHP, then Elixir, subject to representative repositories and concrete adapter-request demand.
