...
5. Harden SwiftPM, Xcode, and Bazel ownership plus conservative Swift symbol evidence. Xcode workspace/project, shared-scheme, and test-plan ambiguity now blocks command selection, while test-local shadows and same-named member calls no longer inflate source evidence. SwiftPM graph ownership and deterministic extension/member evidence remain active pressure.
