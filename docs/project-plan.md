...
9. Start the fifth adapter after the four supported adapters meet the hardening definition of done. The bounded experimental Go spike now covers conventional modules, literal repository-contained `go.work` members, explicit static build targets, and generic top-level function evidence with module-local or target-qualified commands, standard-library tests, all three pinned live roles, shared conformance, golden artifacts, model-consistency locking, and implementation coverage. Promotion remains gated by the generated performance/evidence regression and the checked-in [Go support matrix](go-experimental-support.md).
