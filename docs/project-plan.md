...
11. Start C#/.NET after the bounded Rust spike reaches its intended comparison point. In progress; the experimental adapter audits one static SDK-style test `.csproj` or one unique literal production/test edge, including a selected pair amid unrelated projects, detects bounded xUnit/NUnit/MSTest attributed tests, selects an exact test-project `dotnet test` command, isolates project-owned source, and emits direct unique-type call plus stable concrete-local receiver/one-result assertion evidence across the verified edge. The TDD probe validates receiver precision; the Sharp Cast probe validates deterministic pair selection from four projects and passes 165/165 native tests without unrelated-source leakage. Solution ownership, multiple valid edges, transitive project edges, and evaluated MSBuild ownership remain later slices in the [C# experimental support matrix](csharp-alpha-support.md). The provisional sequence after C#/.NET is Ruby, PHP, then Elixir, subject to representative repositories and concrete adapter-request demand.
