...
11. Start C#/.NET after the bounded Rust spike reaches its intended comparison point. In progress; the experimental adapter audits one static SDK-style test `.csproj` or one unique literal production/test edge, including exact literal multi-target membership and a selected pair amid unrelated projects, reads bounded nearest-file `Directory.Build.props` metadata and static `Directory.Packages.props` versions, detects xUnit/NUnit/MSTest attributed tests, selects an exact test-project `dotnet test` command, isolates project-owned source, and emits direct unique-type calls plus stable concrete-local or exact immutable test-field receiver evidence with one assigned or inline `out var` result assertion. TDD validates local receiver precision, Sharp Cast validates deterministic pair selection and 165/165 native tests, Glob validates inherited-framework selection plus 179 passing tests under the documented local runtime roll-forward, EFCore.CheckConstraints validates central versions plus 118 passing tests, and ErrorOrAspNetCoreExtensions validates literal multi-target ownership plus 49 passing tests. Solution ownership, multiple valid edges, transitive project edges, broader evaluated MSBuild ownership, framework compatibility inference, conditional/overridden central versions, and deeper result flow remain later slices in the [C# experimental support matrix](csharp-alpha-support.md). The provisional sequence after C#/.NET is Ruby, PHP, then Elixir, subject to representative repositories and concrete adapter-request demand.
