...
11. Start C#/.NET after the bounded Rust spike reaches its intended comparison point. In progress; the experimental adapter audits one static SDK-style test `.csproj` or one literal production/test project pair, detects bounded xUnit/NUnit/MSTest attributed tests, selects an exact test-project `dotnet test` command, isolates project-owned source, and emits direct unique-type call evidence across the verified edge. Solution graphs, transitive project edges, and evaluated MSBuild ownership remain later slices in the [C# experimental support matrix](csharp-alpha-support.md). The provisional sequence after C#/.NET is Ruby, PHP, then Elixir, subject to representative repositories and concrete adapter-request demand.
