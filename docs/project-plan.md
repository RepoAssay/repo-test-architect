...
9. Start the fifth adapter after the four supported adapters meet the hardening definition of done. The bounded experimental Go spike is complete for a conventional single `go.mod` module with standard-library tests, shared conformance, golden artifacts, model-consistency locking, and implementation coverage. Promotion remains gated by the checked-in [Go support matrix](go-experimental-support.md).
