...
12. Start Ruby as the next bounded adapter. In progress: the experimental conventional Bundler foundation owns `lib/`, runnable Minitest/RSpec conventions, exact bounded commands, one literal root `.rspec` helper edge within three-edge require/unique-constant evidence, unique-basename fallback, exact source-owned singleton calls, direct immutable constructor-local instance calls, bounded assertion usage, one native fixture, and a generated 400-source/200-test gate. Pinned rubyzip command/constant/receiver pressure and Faraday configured-helper/RSpec/generated-method pressure are regression-backed; a third live role, broader RSpec identity, Rails ownership, and the three promotion roles remain later slices in the [Ruby alpha support matrix](ruby-alpha-support.md). PHP then Elixir remain the provisional next-adapter order.
