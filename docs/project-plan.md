...
12. Start Ruby as the next bounded adapter. In progress: the experimental conventional Bundler foundation owns `lib/`, runnable Minitest/RSpec conventions, exact bounded commands, three-edge literal require/unique-constant evidence with unique-basename fallback, exact source-owned singleton calls with bounded assertion usage, one native fixture, and a generated 400-source/200-test gate. Pinned rubyzip command, constant, call, and assertion pressure is regression-backed; broader RSpec validation, instance receivers, Rails ownership, and the three promotion roles remain later slices in the [Ruby alpha support matrix](ruby-alpha-support.md). PHP then Elixir remain the provisional next-adapter order.
