...
12. Start Ruby as the next bounded adapter. In progress: the experimental conventional Bundler foundation owns `lib/`, one root gemspec or a complete exact named root-gemspec set, runnable Minitest/RSpec conventions, exact bounded commands, literal root `.rspec` and exact per-file `spec_helper` edges within three-edge require/unique-constant evidence, unique-basename fallback, exact source-owned singleton calls, nearest exact constant-owned RSpec `described_class`, direct immutable constructor-local, exact one-line RSpec `let`/`subject`, exact source-factory, and same-group RSpec helper-return instance calls, bounded assertion usage, one native fixture, and a generated 400-source/200-test gate. Pinned rubyzip command/constant/receiver pressure, Faraday configured-helper/RSpec/memoized-receiver/generated-method pressure, Factory Bot class-oriented `described_class` plus memoized-receiver negative pressure, Diplomat complete multi-gemspec/per-file-helper ownership, and Dinie exact helper-return pressure with CGRateS as a lexical-constant negative control are regression-backed. Shared-example binding, partial/path gemspec ownership, Rails ownership, and the formal three promotion roles remain later slices in the [Ruby alpha support matrix](ruby-alpha-support.md). PHP then Elixir remain the provisional next-adapter order.
