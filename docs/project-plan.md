...
11. Start C#/.NET after the bounded Rust spike reaches its intended comparison point. In progress; the experimental adapter audits one static SDK-style test `.csproj` or one unique literal production/test edge, including exact literal multi-target membership, finite project-local target-conditioned package predicates, and a selected pair amid unrelated projects, reads bounded nearest-file `Directory.Build.props` metadata and static `Directory.Packages.props` versions, detects xUnit/NUnit/MSTest attributed tests, selects an exact test-project `dotnet test` command, isolates project-owned source, and emits runnable-body-owned direct unique-type calls with well-known `System` collision guards plus stable concrete-local or exact immutable test-field receiver evidence with bounded result/framework assertions, provenance-backed NUnit Legacy `ClassicAssert`, and one same-class private-static test-helper hop. Seven pinned probes now cover receiver precision, deterministic pair selection, inherited frameworks, central versions, literal multi-target ownership, S7.Net's target-conditioned `System.Memory` dependency and evidence pressure, plus repository-wide routing and NUnit 5 pressure across the official six-project samples; native portability failures remain recorded where the target repository requires unavailable host architecture, runtime, or native libraries. Solution ownership, multiple valid edges, transitive project edges, broader evaluated MSBuild ownership, framework compatibility inference, conditional/overridden central versions, deeper helper reachability, and deeper result flow remain later slices in the [C# experimental support matrix](csharp-alpha-support.md). The provisional sequence after C#/.NET is Ruby, PHP, then Elixir, subject to representative repositories and concrete adapter-request demand.
