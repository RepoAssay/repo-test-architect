...
9. Start the fifth adapter after the four supported adapters meet the hardening definition of done. The bounded experimental Go spike now covers conventional modules, literal repository-contained `go.work` members, and explicit static build targets with module-local or target-qualified commands, standard-library tests, shared conformance, golden artifacts, model-consistency locking, and implementation coverage. Promotion remains gated by the checked-in [Go support matrix](go-experimental-support.md).
