...
21. Extract the first behavior-identical shared audit mechanics. Complete: a private core module owns portable repository paths, changed-path normalization, recursive UTF-8 text reads, explicit symbolic-link skipping, and deterministic path ordering. PHP and Elixir are the only migrated adapters; each supplies its own ignored-name set, nested Composer/Mix pruning callback, and exact file allowlist. Parsers, ownership, frameworks, commands, classification, evidence, and blockers remain adapter-owned, and a shared file index stays deferred until a concrete migration requires it.
22. Profile the exact Swift Package Index Server and Django latency pins. Complete: five-run phase medians and directional CPU profiles show Swift spending 98.2% in evidence/classification through repeated per-pair test scans, while Django spends 65.1% in test parsing/indexing through overlapping lexical consumers. Optimize immutable Swift test facts first, then Python parsed test/support-file reuse in a separate slice. See the [Swift And Python Exact-Pin Phase Profile](swift-python-phase-profile-2026-08.md).
23. Index immutable Swift test facts without changing evidence semantics. Complete: each test's imports, owner facts, masked code, local declarations, assertion bodies, and identifier positions are derived once, while source-specific uniqueness and build ownership remain live. The exact Swift Package Index Server median drops from 14,443 ms to 725 ms and the generated 400/200 check from 439 ms to 81 ms with unchanged canonical SHA, candidate counts, and 265 evidence relationships.
24. Index immutable Python test/support-file facts without changing framework or evidence semantics. Complete: each eligible file's masks, parsed functions, and resolved imports are derived once and reused by runnable-test, pytest-fixture, and framework-client consumers, while fixture visibility, source ownership, route matching, and evidence strength remain live. The exact Django median drops from 3,823 ms to 2,129 ms and test parsing/indexing from 2,488 ms to 844 ms with unchanged canonical SHA, candidate counts, and 4,935 evidence relationships.
