...
14. Start Elixir as a bounded Mix/ExUnit adapter. Foundation complete at experimental maturity: one literal conventional Mix application owns path-matching `lib/` modules and runnable ExUnit tests, emits direct aliased/fully qualified module-call evidence plus conservative filename evidence, and selects `mix test` only when ownership and startup are proven. Native fixture, shared conformance, implementation coverage, golden artifacts, packaging, and generated 400-source/200-test pressure are wired. Next, run a blind conventional-library audit and turn concrete gaps into bounded regression-backed slices; portfolio analysis begins only after Elixir reaches its intended comparison point.
