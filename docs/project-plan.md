...
8. Complete the broader cross-adapter trust pass and cut the next public alpha after the Swift, live-repository, scorecard, and exact-commit release gates pass. Trust pass complete: shared conformance now covers every project-aware downstream artifact and blocked JSON round trips for all four adapters, with canonical project-summary ranking and absent verification-command serialization corrected. The next alpha is a separate exact-commit release action.
