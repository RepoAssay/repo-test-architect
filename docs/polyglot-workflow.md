...
Detection finds project roots from marker files such as `package.json`, `pyproject.toml`, `Gemfile`, `composer.json`, `go.mod`, `Cargo.toml`, `.csproj`, `pom.xml`, or `build.gradle.kts`, records likely ecosystems and languages, and lists matching adapter IDs.
