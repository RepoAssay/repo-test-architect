...
This matrix defines the bounded private-alpha support claim for the Kotlin/JVM adapter. The supported unit is a conventional Gradle or Maven JVM module root, not every project that happens to contain Kotlin or Java.
...
Unsupported test frameworks, Android markers, missing standard root source sets, and aggregate roots are emitted as blockers instead of silently upgraded to supported coverage.
...
4. Aggregate, Android, multiplatform, and unsupported-framework shapes remain visibly blocked or excluded.
