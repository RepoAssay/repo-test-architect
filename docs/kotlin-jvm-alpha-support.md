...
This matrix defines the bounded public-alpha support claim for the Kotlin/JVM adapter. The supported unit is a conventional Gradle or Maven JVM module root, a complete literal root-declared Gradle aggregate, a complete recursively literal Maven reactor whose owned modules have standard JVM source sets, one conventional Kotlin Multiplatform Gradle module, or a settings-owned all-KMP aggregate whose source modules each have exactly one literal default or named JVM target—not every project that happens to contain Kotlin or Java.
...
- Maven profile-activated modules, property-expanded or escaping module paths at any reactor level, inherited dependencies, non-compile/optional/exclusion-bearing transitive edges, dynamic coordinates, and nonstandard module layouts
...
For Maven reactors, the adapter recursively follows literal repository-contained `<module>` paths whose direct POMs have static coordinates. Any property-expanded, absolute, escaping, missing, or dynamically coordinated declaration at any owned level makes the entire aggregate incomplete: the audit keeps only conventional root-owned sources, suppresses the aggregate test command, and reports the exact ownership boundary. Project detection leaves incomplete nested child reactors visible as separate projects rather than partially collapsing them into the root. Profile-activated and plugin-configuration module lists remain separate projects and do not invalidate a complete reactor graph.
...
4. Conventional Gradle and recursively literal Maven module graphs are dependency-qualified through direct or explicitly exported edges with cycle protection; supported Kotest and Spock specs require directly visible Gradle/JUnit Platform execution and supported TestNG files require direct conventional execution; single-module and settings-owned all-KMP JVM ownership preserves common-versus-target source-set reachability, follows direct visibility and cycle-safe literal `api(project(...))` exports without leaking `implementation` edges, and derives module-qualified `<targetName>Test` tasks; custom mappings, computed reactor paths, non-exported dependencies, Android, broader KMP shapes, and unsupported framework/spec shapes remain visibly blocked or excluded.
