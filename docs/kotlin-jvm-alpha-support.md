...
This matrix defines the bounded private-alpha support claim for the Kotlin/JVM adapter. The supported unit is a conventional Gradle or Maven JVM module root, or a conventional Gradle aggregate whose included modules have standard JVM source sets—not every project that happens to contain Kotlin or Java.

## Current Common-Pattern Coverage

| Area | Current support | Evidence used |
| --- | --- | --- |
| Project shapes | Single-module or directly audited Gradle/Maven JVM modules; conventional Gradle aggregates | root/module build files, settings `include(...)` declarations, and standard JVM source sets |
...
- Maven aggregate/reactor-root ownership and cross-module evidence
- Gradle composite builds, custom `projectDir` remaps, transitive project dependencies, and modules without conventional build files/source sets
...
1. Gradle Kotlin DSL, Gradle Groovy DSL, Maven, Gradle wrapper, and Maven wrapper behavior is fixture-locked.
...
4. Conventional Gradle module graphs are dependency-qualified; custom mappings, Maven aggregates, Android, multiplatform, and unsupported-framework shapes remain visibly blocked or excluded.
...
1. Extend owned module graphs to Maven reactors and carefully bounded transitive Gradle dependencies.
