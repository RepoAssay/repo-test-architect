...
`audit_repo` is for an explicitly selected single project root and adapter. It accepts an optional `adapterId`, optional repository-relative `changedPaths`, and optional `goTarget: { goos, goarch, tags }`, and defaults to `javascript` for compatibility. `analyze_repository` and `audit_projects` accept the same Go target object for project-aware scans. The current supported adapters are `csharp`, `javascript`, `go`, `kotlin`, `python`, `rust`, and `swift`; bounded Go covers standalone modules, literal repository-contained `go.work` members, and explicit static build targets in [Go Alpha Support](go-alpha-support.md), Kotlin/JVM is bounded by [Kotlin/JVM Alpha Support](kotlin-jvm-alpha-support.md), Rust is bounded by [Rust Alpha Support](rust-alpha-support.md), and C# is bounded by [C# Alpha Support](csharp-alpha-support.md).
