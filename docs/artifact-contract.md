...
- C# emits `csharp-symbol-reference` with `direct` strength for a uniquely owned class, record, or struct used through an exact static member call or constructor in a runnable attributed test. Calls inside an `Assert.*` or `.Should(...)` statement use `asserted`; other exact calls use `called`. A unique source/test basename match may fall back to the shared `filename-convention` naming evidence.
