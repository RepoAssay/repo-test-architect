...
- C# emits `csharp-symbol-reference` with `direct` strength for a uniquely owned class, record, or struct used through an exact static member call, constructor, or stable concrete local receiver in a runnable attributed test. Direct receiver calls and one stable local result consumed by `Assert.*` or `.Should(...)` use `asserted`; other exact calls use `called`. Reassignment, interface/field/helper identity, and deeper flow are rejected. A unique source/test basename match may fall back to the shared `filename-convention` naming evidence.
...
- Rust emits `rust-symbol-reference` with `direct` strength for a unique source function called from the same file's runnable inline `#[cfg(test)]` module or through an exact package-name and module-qualified `use` binding in a runnable `tests/` integration test. Calls use `called`; calls inside built-in assertion macros use `asserted`. Unused imports, other crate names, ambiguous modules, comments, strings, workspaces, and custom harnesses do not contribute evidence.
