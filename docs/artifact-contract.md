...
- C# emits `csharp-symbol-reference` with `direct` strength for a uniquely owned class, record, or struct used through an exact static member call, constructor, stable concrete local receiver, or exact `private readonly` concrete test-class field in a runnable attributed test. Supported fields use exact inline construction or one assignment in the class's sole parameterless constructor. Direct receiver calls and one stable local result consumed by `Assert.*` or `.Should(...)` use `asserted`; other exact calls use `called`. Reassignment, mutable/static/interface/property/inherited/helper field identity, and deeper flow are rejected. A unique source/test basename match may fall back to the shared `filename-convention` naming evidence.
