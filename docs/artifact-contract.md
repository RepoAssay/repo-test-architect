...
- Go emits `go-symbol-reference` with `direct` strength and `called` usage for unique top-level functions and receiver-type/method pairs called through a unique explicitly typed local binding; exact type construction is `referenced`, while `go-source-dependency` remains one bounded same-package function hop with `indirect` strength
