...
- Elixir emits `elixir-module-reference` with `direct` strength when an app-owned runnable ExUnit test body calls one uniquely owned module or protocol through its fully qualified name, one unambiguous exact alias, or an exact grouped alias. A direct `assert` or `refute` expression containing that call emits `asserted`; other direct calls emit `called`. Calls outside extracted test bodies and duplicate owned FQNs receive no direct evidence. A conventional source/test path match may fall back to shared `filename-convention` evidence with `naming` strength.
...
- Ruby emits `ruby-constant-reference` when a runnable conventional Minitest or RSpec file visibly references a uniquely owned space-indented class/module constant reached through at most three exact literal repository-owned load edges. A safe literal root `.rspec --require` may contribute one uniquely resolved root or `spec/` helper edge. In a runnable `spec/**/*_spec.rb` file, an exact zero-indented literal `require "spec_helper"` or `require("spec_helper")` resolves specifically to `spec/spec_helper.rb` and consumes one edge; alternate names, nesting, computation, explicit `spec/` paths, missing helpers, and Minitest files do not gain that runner-owned path. Other bare `require` calls resolve below `lib/` only when one root gemspec or a complete exact named set of root gemspecs is statically included by the root Gemfile, and `require_relative` resolves from the owning file. A multi-gemspec set is complete only when exact top-level `gemspec name:` declarations match unique static names declared by every root gemspec; partial, duplicate, dynamic, nested, path, and alternate syntax remains blocked. One direct source edge has `direct` strength; helper or entrypoint reachability has `referenced` strength. Inside a conventional runnable test/example body, an exact constant-qualified method declared directly as `def self.name`, inside a direct `class << self`, or as `.new` backed by that class's direct `initialize` emits `called`. In RSpec, `described_class` may replace that receiver only under its nearest containing static constant-first `describe`; nested string groups or contexts inherit the owner and a nearer exact constant group overrides it. A local bound exactly once from that direct constructor may also emit `called` for a later direct call to a method declared on the same class, provided the class does not override `.new` and the receiver is neither reassigned nor block-shadowed. The same direct instance rule applies to an exact one-line RSpec `let` or `subject` constructor receiver in the same normal group scope, one source singleton factory whose whole body is `new(...)` or `self.new(...)`, or one same-file RSpec group helper whose whole body is `Constant.new(...)` or exact group-owned `described_class.new(...)`. Nearest declaration wins, unsupported inner helper or memo declarations block exact outer bindings, and named subjects expose both their name and `subject`. A same-line selected Minitest assertion or RSpec `expect`, or one unique unreassigned local result later consumed by one of those assertions, emits `asserted`. Setup hooks, aliases, Minitest/cross-file/shared/chained/wrapped/multi-statement/computed helper or factory returns, eager or multiline memos, chained/block-bearing/aliased/computed memo constructors, implicit matchers, generated readers or delegators, inherited/mixed-in methods, dynamic group expressions, dynamic dispatch, deferred lambdas, reassigned receivers/results, and custom assertion-shaped methods do not upgrade usage. Reopened constants with multiple reachable owners, dynamic or nested RSpec configuration, ambiguous configured helpers, dynamic loads, deeper graphs, comments, strings, partial constant names, and unqualified nested constants do not contribute evidence. A unique source/test basename may fall back to shared `filename-convention` evidence with `naming` strength.
