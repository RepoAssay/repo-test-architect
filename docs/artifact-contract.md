...
- Elixir emits `elixir-module-reference` with `direct` strength when a conventional runnable ExUnit test calls an exactly owned module through its fully qualified name or one unambiguous exact alias. A direct `assert` or `refute` expression containing that call emits `asserted`; other direct calls emit `called`. A conventional source/test path match may fall back to shared `filename-convention` evidence with `naming` strength.
