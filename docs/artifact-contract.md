...
- Go emits `go-symbol-reference` with `direct` strength and `called` usage for unique top-level functions and receiver-type/method pairs called through a unique concrete local binding; the receiver type can be explicit or come from an exact, simple, non-generic constructor result position. Usage upgrades to `asserted` when the call is inside a bounded standard-library failure condition or an exact Testify assertion, or when one unique `:=` result binding is consumed there. Exact default, named, or dot imports can qualify exported external-package symbols, exact type construction is `referenced`, and `go-source-dependency` remains one bounded function hop with `indirect` strength while preserving the entrypoint's `viaUsage`. Cross-package hops additionally require an exact module-local import, exported unique function, and a source file with exactly one callable declaration
