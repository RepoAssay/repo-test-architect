...
| [Apache Maven Surefire](https://github.com/apache/maven-surefire) root | `8dcf263f808c15b00a7064ec6ea3f9268c1d4b51` | multi-level Maven reactor, Java, JUnit 4/5 | high; `mvn test`; no blockers | 27 | 139 | 137 | 556 ms median |
...
| [Spatial-K](https://github.com/maplibre/spatial-k) root | `48b2dd7ea871b8ae29f58ed5d964c95b0618fa26` | four-module KMP aggregate with literal common-source-set `api` and `implementation` project edges | high; four module-qualified `jvmTest` tasks; no blockers | 0 | 39 | 19 | about 48 ms |
...
Maven Surefire exercised recursively literal reactor ownership at realistic scale. The post-Swift live-report refresh exposed that the complete `surefire-providers` aggregator was being rejected after reactor hardening even though both nested module paths were literal, contained, directly present, and statically coordinated. Recursive ownership restores the root command and now finds 356 evidence relationships across 12 owned test locations; the two newly owned provider modules contribute seven covered targets, three untested targets, seven low-value skips, and eight evidence relationships. The original 143 cross-module relationships remain dependency-qualified. Profile-owned modules, computed or incomplete nested expansion, and inherited/dynamic dependency inference remain outside the result, while embedded integration-test sample POMs stay separate project-detection candidates.
...
Spatial-K provided the positive exported-transitive source-set proof at the last pinned commit before its target declarations moved into convention plugins. The four settings-owned KMP modules expose literal JVM targets and a mix of `commonMain api(project(...))` and `implementation(project(...))` edges. Cycle-safe API traversal increases exact evidence from 238 to 296 relationships and cross-module evidence from 13 to 71, primarily by admitting `turf/commonTest` references to `geojson` and `units` through `turf`'s public common API. `testutil`'s internal implementation edges do not become exports.
...
- bounded Maven reactor ownership to complete recursively literal declarations with direct static child POMs at every owned level; unsafe or unresolved expansion still blocks partial child ownership and command selection while incomplete nested projects remain separately detectable
...
- added cycle-safe common/JVM traversal through literal KMP `api(project(...))` edges while retaining `implementation` isolation, validated against Spatial-K
...
- used the post-Swift live refresh to recover Surefire's complete literal nested provider aggregator without admitting computed, missing, escaping, or dynamically coordinated descendants
...
- broader Maven profile/computed reactor execution and ownership, inherited/dynamic dependencies, Gradle composite builds, computed includes, custom module mappings, and non-exported transitive project dependencies; incomplete Maven and Gradle aggregate shapes are explicitly blocked rather than partially audited
...
The live probes support conventional single-module or directly selected Gradle/Maven JVM roots, settings-owned conventional Gradle aggregates, complete recursively literal Maven reactors, cycle-safe traversal through explicitly exported Gradle/Maven module edges, and single-module or settings-owned all-KMP builds with conventional common/JVM source sets and exactly one literal default or named JVM target per source module. KMP cross-module evidence follows direct source-set visibility plus cycle-safe literal `api(project(...))` exports while preserving common-versus-JVM reachability and excluding internal implementation edges. Supported evidence uses JUnit 4, JUnit 5, `kotlin.test`, the documented conventional-Gradle Kotest common-spec and Spock feature variants, or method-level TestNG under direct conventional Maven/Gradle execution. They do not support a broad Kotlin/Java, Kotlin Multiplatform, Kotest, Spock, or TestNG ecosystem claim. The exact boundary is normative in [Kotlin/JVM Alpha Support](kotlin-jvm-alpha-support.md).
