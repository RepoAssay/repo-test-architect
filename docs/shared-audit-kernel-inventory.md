...
The [August 2026 exact-pin phase profile](swift-python-phase-profile-2026-08.md) is complete. Swift spends 14,187 ms of its 14,443 ms median in evidence/classification, driven by repeated test import scanning, masking, and symbol analysis across source/test pairs. Django spends 2,488 ms of its 3,823 ms median in test parsing/indexing and 858 ms in evidence/classification, with repeated masking, function parsing, and import binding across runnable-test, fixture, and framework-client consumers. Traversal accounts for only 17 ms and 154 ms respectively, so neither adapter should migrate traversal for performance reasons. The next two slices are adapter-local immutable Swift test facts followed by a Python parsed test/support-file cache.
