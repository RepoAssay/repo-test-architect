...
Progress: all seven supported adapters now have deterministic 400-source/200-test semantic and performance regression gates with adapter-specific audit timing, covered/untested counts, and evidence-relationship counts. Rust carries the same generated pressure, including an exact skipped crate-root wiring invariant. serde_json, Starship, and ripgrep fill its conventional, framework-heavy, and difficult-ownership roles; exact crate-root re-export evidence and test-only module exclusion fixed the two live precision gaps. These synthetic ceilings protect local and CI performance, while all 21 supported-adapter pinned cases now have repeated-run distributions and stable canonical audit digests.

The experimental Ruby foundation now has the same generated 400-source/200-test pressure before broader live-repository expansion. Its boundary is one conventional Bundler project, `lib/` ownership, one root gemspec or a complete exact named root-gemspec set, Minitest/RSpec discovery, exact bounded commands, one uniquely owned literal root `.rspec` helper edge, exact literal require/unique-constant evidence through at most three repository-owned edges, runnable-body-owned singleton calls, nearest exact constant-owned RSpec `described_class`, and direct immutable constructor-local or exact one-line RSpec `let`/`subject` instance calls with inline or one-stable-result Minitest/RSpec assertion usage. rubyzip locks Minitest command and receiver pressure; Faraday locks configured-helper, exact memoized receiver, and generated-method boundaries with 639 passing native examples and 45 relationships; Factory Bot locks class-oriented RSpec identity and a no-change memoized-receiver comparison with 764 passing adapter-selected examples; Diplomat locks complete two-gemspec aggregate ownership with 281 passing examples and an unchanged naming graph. Eager or multiline memos, per-file test-helper loads, factory/helper receivers, shared examples, deeper flow, partial/path gemspec ownership, and Rails remain future work.
