...
The experimental Ruby foundation now has the same generated 400-source/200-test pressure before broader live-repository expansion. Its boundary is one conventional Bundler project, `lib/` ownership, Minitest/RSpec discovery, exact bounded commands, and exact literal require/unique-constant evidence through at most three repository-owned edges with naming fallback. rubyzip pressure locks the first evidence slice; usage/assertion semantics, broader RSpec pressure, and Rails ownership remain future slices.
