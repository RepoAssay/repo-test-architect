...
9. Reconsider another adapter or native generation. Complete through the [August 2026 Adapter Portfolio Analysis](adapter-portfolio-analysis-2026-08.md): Elixir promotion and the [shared-kernel inventory](shared-audit-kernel-inventory.md) are complete; add development-only Swift/Python phase timing next, then make the first byte-preserving PHP/Elixir traversal extraction, evaluate but do not ship one bounded executor loop after the measured trust work, and keep another ecosystem demand-gated. C++/CMake and Dart/Flutter remain discovery candidates rather than an inferred queue.

Run `npm run corpus:check` to validate corpus completeness, pins, report links, and scorecard states. Use `npm run corpus:measure -- --case <case-id> --checkout <path>` against an exact pinned Git checkout to record at least three canonical audits, raw durations, their median, the evidence count, and normalized SHA-256 digest. All 30 current supported-adapter cases pass every scorecard area after standardized measurements, including Jason, Plug, and Absinthe. A case may preserve bounded adapter audit options, such as an explicit Go build target, in the manifest. New or repinned cases remain `pending` until their own measurements pass; historical timings alone never silently pass the gate.
