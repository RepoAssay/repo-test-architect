...
Progress: all six supported adapters now have deterministic 400-source/200-test semantic and performance regression gates with adapter-specific audit timing, covered/untested counts, and evidence-relationship counts. Experimental Rust now carries the same generated pressure, including an exact skipped crate-root wiring invariant. Ripgrep covers its difficult-ownership role, and Starship covers its framework-heavy role after exact test-only module ownership removed one false candidate and 94 inflated relationships; one conventional-library/service role remains. These synthetic ceilings protect local and CI performance, while all 18 supported-adapter pinned cases now have repeated-run distributions and stable canonical audit digests.
...
Run `npm run corpus:check` to validate corpus completeness, pins, report links, and scorecard states. Use `npm run corpus:measure -- --case <case-id> --checkout <path>` against an exact pinned Git checkout to record at least three canonical audits, raw durations, their median, the evidence count, and normalized SHA-256 digest. All 18 current cases pass every scorecard area after standardized measurements. A case may preserve bounded adapter audit options, such as an explicit Go build target, in the manifest. New or repinned cases remain `pending` until their own measurements pass; historical timings alone never silently pass the gate.
