...
The experimental Ruby foundation now has the same generated 400-source/200-test pressure before broader live-repository expansion. Its boundary is one conventional Bundler project, `lib/` ownership, one root gemspec or a complete exact named root-gemspec set, Minitest/RSpec discovery, exact bounded commands, uniquely owned literal root `.rspec` and exact per-file `spec_helper` edges, exact literal require/unique-constant evidence through at most three repository-owned edges, runnable-body-owned singleton calls, nearest exact constant-owned RSpec `described_class`, and direct immutable constructor-local, exact one-line RSpec `let`/`subject`, exact source-factory, or same-group RSpec helper-return instance calls with inline or one-stable-result Minitest/RSpec assertion usage. rubyzip locks Minitest command and receiver pressure; Faraday locks configured-helper, exact memoized receiver, and generated-method boundaries with 639 passing native examples and 45 relationships; Factory Bot locks class-oriented RSpec identity and a no-change memoized-receiver comparison with 764 passing adapter-selected examples; Diplomat locks complete two-gemspec aggregate ownership plus exact per-file helper loading with 281 passing examples and 26 relationships; Dinie locks exact helper returns with 579 passing examples and 47 unchanged relationships, while CGRateS preserves its graph as a lexical-constant negative control. Eager or multiline memos, shared examples, cross-file or computed helpers/factories, deeper flow, partial/path gemspec ownership, dynamic source fan-out, and Rails remain future work.
