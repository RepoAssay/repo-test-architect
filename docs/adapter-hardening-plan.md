...
The experimental Ruby foundation now has the same generated 400-source/200-test pressure before broader live-repository expansion. Its boundary is one conventional Bundler project, `lib/` ownership, Minitest/RSpec discovery, exact bounded commands, one uniquely owned literal root `.rspec` helper edge, exact literal require/unique-constant evidence through at most three repository-owned edges, and runnable-body-owned singleton calls with inline or one-stable-result Minitest/RSpec assertion usage. rubyzip locks Minitest command and usage pressure; Faraday locks RSpec configured-helper ownership with 639 passing native examples and 32 exact relationships while leaving memoized receivers, helpers, shared examples, and dynamic eager loading uncredited. Direct immutable constructor-to-local receivers, broader helpers, and Rails ownership remain future work.
