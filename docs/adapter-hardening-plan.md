...
Run `npm run corpus:check` to validate corpus completeness, pins, report links, and scorecard states. Use `npm run corpus:measure -- --case <case-id> --checkout <path>` against an exact pinned Git checkout to record at least three canonical audits, raw durations, their median, the evidence count, and normalized SHA-256 digest. All 27 supported-adapter cases pass every scorecard area after standardized measurements; Jason, Plug, and Absinthe now form a complete experimental Elixir cohort with all 21 areas pending fresh measurement and review. A case may preserve bounded adapter audit options, such as an explicit Go build target, in the manifest. New or repinned cases remain `pending` until their own measurements pass; historical timings alone never silently pass the gate.
