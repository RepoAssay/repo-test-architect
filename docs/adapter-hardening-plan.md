...
The experimental Ruby foundation now has the same generated 400-source/200-test pressure before live-repository expansion. Its initial boundary is one conventional Bundler project, `lib/` ownership, Minitest/RSpec discovery, exact bounded commands, and naming-only evidence; direct require/constant evidence and Rails ownership remain future slices.
