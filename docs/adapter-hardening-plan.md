...
8. Run the broader cross-adapter trust pass, then cut the next public alpha only after the Swift, live-repository, scorecard, and exact-commit release gates pass. Trust pass complete: all four adapters now share full downstream conformance and blocked-pipeline JSON invariants; project-summary ranking and absent-command serialization mismatches were fixed. The next alpha remains a separate exact-commit release action.
