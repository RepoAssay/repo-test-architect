...
The experimental Ruby foundation now has the same generated 400-source/200-test pressure before broader live-repository expansion. Its boundary is one conventional Bundler project, `lib/` ownership, Minitest/RSpec discovery, exact bounded commands, one uniquely owned literal root `.rspec` helper edge, exact literal require/unique-constant evidence through at most three repository-owned edges, runnable-body-owned singleton calls, nearest exact constant-owned RSpec `described_class`, and direct immutable constructor-local instance calls with inline or one-stable-result Minitest/RSpec assertion usage. rubyzip locks Minitest command and receiver pressure; Faraday locks configured-helper ownership and generated-method exclusions with 639 passing native examples and 32 exact relationships; Factory Bot locks class-oriented RSpec identity with 764 passing adapter-selected examples and four reviewed usage improvements. `let`/`subject`, factory/helper receivers, shared examples, deeper flow, broader ownership, and Rails remain future work.
