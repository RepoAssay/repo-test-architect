...
The experimental Ruby foundation now has the same generated 400-source/200-test pressure before broader live-repository expansion. Its boundary is one conventional Bundler project, `lib/` ownership, Minitest/RSpec discovery, exact bounded commands, exact literal require/unique-constant evidence through at most three repository-owned edges, and runnable-body-owned singleton calls with inline or one-stable-result Minitest/RSpec assertion usage. rubyzip pressure locks both evidence slices; broader RSpec pressure, instance receivers, helpers, and Rails ownership remain future work.
