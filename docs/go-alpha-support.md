...
| Test provenance | exact co-located `foo.go`/`foo_test.go` naming, unique same-package top-level functions/types, generic top-level function calls with inferred or explicit type arguments, value or pointer receiver methods called through a unique explicitly typed local binding, exact external `package_test` imports through the module path using default, named, or dot imports, and one called same-package source dependency | filename ownership, package declarations, literal imports/aliases, exported external identifiers, balanced generic declarations/instantiations, receiver type plus method ownership, unambiguous short or `var` declarations, visible calls or type construction, and one unqualified unique top-level function call from a directly called source file |
...
`go-symbol-reference` evidence is `direct` with `called` usage for a uniquely owned top-level function call or receiver method call, or `referenced` for a uniquely owned type construction. Receiver methods require a unique receiver-type/method declaration and a unique explicit test binding such as `client := Client{}` or `var client *Client = &Client{}` before `client.Authorize(...)`; exact external-package default, named, and dot imports are supported. External evidence is limited to exported identifiers, while blank imports, unrelated paths, and test-local shadows contribute nothing. Constructor-inferred bindings, reassignment, and duplicate or shadowed bindings are not direct method evidence. `go-source-dependency` is `indirect` with `viaUsage: called` for one same-package, same-directory, unique top-level function dependency from a directly called source file; it never propagates to a second source hop. Filename convention remains weaker `naming` evidence.
...
After adding explicitly typed receiver-method evidence, all three pinned corpus roles retained their candidate counts, evidence counts, and canonical audit digests. The live pass also caught and fixed an initial test-method shadow-set regression before any baseline was updated.
