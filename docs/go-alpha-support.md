...
| Test provenance | exact co-located `foo.go`/`foo_test.go` naming, unique same-package top-level functions/types, generic top-level function calls with inferred or explicit type arguments, value or pointer receiver methods called through a unique explicitly typed local binding, exact external `package_test` imports through the module path, and one called same-package source dependency | filename ownership, package declarations, literal imports/aliases, balanced generic declarations/instantiations, receiver type plus method ownership, unambiguous short or `var` declarations, visible calls or type construction, and one unqualified unique top-level function call from a directly called source file |
