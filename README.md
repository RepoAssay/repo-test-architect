...
- audit one conventional Mix application with ExUnit through an experimental bounded Elixir adapter
...
- `elixir` (experimental): one conventional Mix application with literal app ownership, path-matching `lib/` modules, conventional ExUnit modules, direct aliased or fully qualified module-call evidence, conservative filename evidence, and a blocker-free `mix test` command; see [Elixir experimental support](docs/elixir-alpha-support.md)
