...
- audit JavaScript/TypeScript, Python, Ruby, and Swift projects through supported adapters
...
- audit one conventional Composer/PSR-4 project with PHPUnit through an experimental bounded PHP adapter
...
- `php` (experimental): one root Composer project with literal string-valued PSR-4 source/test ownership, statically declared PHPUnit, exact bounded commands, conventional runnable test classes, direct imported class calls, and conservative basename fallback; see [PHP experimental support](docs/php-alpha-support.md)
...
Project detection reports PHP roots through the experimental adapter and still reports unsupported Elixir roots so clients can distinguish "not audited yet" from "not present."
