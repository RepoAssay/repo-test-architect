...
- audit one conventional Mix application with app-owned ExUnit tests, exact MixProject/Mixfile ownership, and bounded test-body evidence through an experimental Elixir adapter
...
- `elixir` (experimental): one conventional Mix application with literal app and exact MixProject/Mixfile ownership, conventional or app-prefixed primary modules and protocols with bounded acronym and terminal-plural preservation, app-owned ExUnit modules with static startup options, test-body-scoped direct fully qualified or exact-alias calls, conservative filename evidence, and a blocker-free `mix test` command; see [Elixir experimental support](docs/elixir-alpha-support.md)
...
The versioned `evals/validation-corpus.json` manifest records one conventional library or service, one framework-heavy application, and one difficult ownership graph per adapter. Each record carries the shared detection, ownership, command, evidence, ranking, stability, and performance scorecard. All 27 current pinned cases pass every scorecard area; a new or repinned case stays `pending` until it has been rerun under the standardized hardening review. A reviewed command may be `null` when the adapter correctly withholds unsafe execution. Cases may also carry bounded adapter audit options, such as an explicit Go build target, so repeated measurements remain host-independent.
