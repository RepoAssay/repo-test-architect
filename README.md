...
- audit one conventional SDK-style C# test project or one unique literal production/test project edge, including a literal multi-target pair or a pair amid unrelated projects, bounded nearest-file build and central-package props, exact immutable test-field receivers, and inline `out`-result assertions, through an experimental bounded .NET adapter
