...
- `rust` (experimental): conventional Cargo packages and literal repository-contained workspace members using the built-in `#[test]` harness, inline `#[cfg(test)]` modules, exact exclusion of external test-only module graphs, and exact crate-module imports from `tests/`; see the [Rust experimental support matrix](docs/rust-alpha-support.md)
