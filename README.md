...
- `ruby` (experimental): one conventional Bundler project with `lib/` sources, one root gemspec or a complete exact named root-gemspec set, runnable Minitest or RSpec conventions, exact bounded commands, root `.rspec` literal helper loading, three-edge literal require/unique-constant evidence, exact singleton calls, exact constant-owned RSpec `described_class`, direct immutable constructor-local and one-line RSpec `let`/`subject` receivers, bounded assertion usage, conservative basename fallback, and explicit Rails/mixed-runner blockers; see the [Ruby alpha support matrix](docs/ruby-alpha-support.md)
