...
- audit one conventional SDK-style C# test project or one unique literal production/test project edge, including a pair amid unrelated projects, bounded nearest-file `Directory.Build.props` metadata, exact immutable test-field receivers, and inline `out`-result assertions, through an experimental bounded .NET adapter
...
- `csharp` (experimental): one static SDK-style test project or one unique literal production/test project edge, including a pair selected amid unrelated projects, with bounded nearest-file `Directory.Build.props` metadata, xUnit, NUnit, or MSTest attributed tests, exact project-file commands, direct type calls, bounded concrete-local or immutable test-field receivers, and one-hop assigned or inline `out var` result assertions; see the [C# experimental support matrix](docs/csharp-alpha-support.md)
