...
describe("model consistency runner", () => {
  ...
  it("passes every checked-in model consistency scenario", () => {
    ...
    assert.deepEqual(
      ...
      [
        ...
        ["kotlin-multiplatform-exported-module-graph-plan", []],
        ["kotlin-multiplatform-jvm-plan", []],
        ["kotlin-multiplatform-module-graph-plan", []],
        ["kotlin-multiplatform-named-jvm-plan", []],
        ["node-ava-basic-plan", []],
  ...
  it("summarizes locked-field results for model profile comparisons", () => {
    ...
    assert.deepEqual(summary.summary, {
      scenarioCount: 58,
      passedScenarioCount: 58,
      failedScenarioCount: 0,
      checkedFieldCount: 513,
      failureCount: 0
    });
    assert.equal(summary.scenarios[20].scenarioId, "node-ava-basic-plan");
    assert.equal(summary.scenarios[20].status, "passed");
  ...
  it("flags drift, missing scenarios, and unexpected scenarios between summaries", () => {
    ...
    assert.deepEqual(comparison.summary, {
      ...
      checkedFieldDelta: -358,
      failureDelta: 3
    });
    assert.deepEqual(
      comparison.scenarios.map((scenario) => [scenario.scenarioId, scenario.alignment]),
      [
        ["csharp-sdk-xunit-basic-plan", "drifted"],
        ["express-supertest-plan", "missing"],
        ["go-build-target-basic-plan", "aligned"],
        ["go-testing-basic-plan", "aligned"],
        ["go-workspace-checkout-plan", "aligned"],
        ...
        ["kotlin-multiplatform-exported-module-graph-plan", "aligned"],
        ["kotlin-multiplatform-jvm-plan", "aligned"],
        ["kotlin-multiplatform-module-graph-plan", "aligned"],
        ["kotlin-multiplatform-named-jvm-plan", "aligned"],
